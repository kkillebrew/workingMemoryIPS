...
%  Changes KWK made:
% ********** 1 (blue) is for nothing changed 4 (red) is for something changed
...
% ON 2/28/14:
%  -  added in params to block structure to include multiple cue types
%  -  changed the color of the cue to handle multiple stim
%  type conditions
%  -  added functionality to allow for multiple inter stimulus distances
%  between differnt stimulus types
%  -  changed accuracty calculations to account for multiple stim types
...
params.setup = 'ryan_macair';
switch params.setup
    case 'scanning_laptop'
        % *** this is where we could add details for a different setup
        % monitor dimensions
        params.mon_width_cm = 37; % height X cm
        ...
        % name of the practice button box
        params.kb_dev_id = kb_id(strcmp(kb_names,'Trainer (R1053)')); % use -1 for all devices, or specify a specific device
        ...
        % size
        params.stim.letters.letter_size = 90; % size of letters through TextSize
        
        % Added this here to change easily between systems
        % 1 (blue) is for nothing changed 4 (red) is for something changed
        params.resp_keys  = KbName({'1!' '4$'}); % subject response key(s) index for KbCheck, etc.
    ...
    case 'kyle_lab_comp'
        ...
        % what keyboard to listen to
        [kb_id, kb_names] = GetKeyboardIndices;
        params.kb_dev_id = kb_id(strcmp(kb_names,'Apple Internal Keyboard / Trackpad')); % use -1 for all devices, or specify a specific device
...
params.blocks.memory_reps = 1; % number of reps for each unique memory block (determined by a cross of other parameters, like set size and task-type)

params.blocks.set_size = 3; % list of all possible set sizes (equally drawn from)(number of stim)
...
params.trials_per_block = 1; % how many trials make up a single block?

params.blocks.stimtype = [1 2];  % how many stimulus types are there (verbal and visual)
...
params.cue.color = [255 255 255;0 0 0]; % RGB value (0-255) of block cue and RGB value of second stimtype block cue
...
% stimulus timing
params.prefix_dur  = 1; % duration of prefixation time (before trial 1) in seconds
params.postfix_dur = 1; % duration of final fixation period (after last trial) in seconds
params.block_cue_dur       = 1; % duration of block-cue (cue on ONLY) in seconds
params.block_cue_blank_dur = 1; % duration of block-cue blank (AFTER cue) in seconds
params.sample_dur = 0.100; % sample duration in seconds
params.delay_dur  = 0.900; % delay duration in seconds
params.probe_dur  = 1; % probe duration in seconds
...
params.hemifield_eccen_deg = 5; % how many de rees should each "field" of stimuli be offset to each side?
params.stimtype = {'letters' 'orientation'}; % type of stimulus (currently, for entire run)
...
for i=1:length(params.stimtype)
    switch params.stimtype{i}
        case 'orientation'
            ...
            params.stim.orientation.bar_length_deg = 0.9; % deg
            params.stim.orientation.bar_width_deg = 0.33; % deg
            
            params.stim.orientation.min_dist = 1.1 * params.stim.orientation.bar_length_deg; % min distance between two stimuli within the same hemifield
        ...
        case 'color'
            ...
            params.stim.color.min_dist = 1.1 * params.stim.color.width_deg; % min distance between two stimuli within the same hemifield
        ...
        case 'squares'
            ...
            params.stim.squares.min_dist = 1.1 * sqrt(2*params.stim.squares.width_deg^2); % min distance between two stimuli within the same hemifield
        ...
        case 'letters'
            ...
            params.stim.letters.min_dist = 2; % min distance between two stimuli within the same hemifield
        otherwise
...
cue.rect(3,:) = [xc-cue.halfwidth_pix-2 yc-cue.halfwidth_pix-2 xc+cue.halfwidth_pix+2 yc+cue.halfwidth_pix+2]; % rect for a PASSIVE (CENTERED) block cue
...
[tmp1, tmp2, tmp3] = meshgrid(params.blocks.set_size,params.blocks.mem_side,params.blocks.stimtype);
unique_exp_blocks = [tmp1(:) tmp2(:) tmp3(:)]; % unpack meshgrid output
all_exp_blocks = repmat(unique_exp_blocks,params.blocks.memory_reps,1); % account for expeirmental block repetitions
% blocks.stimtype = repmat({params.stimtype},size(all_exp_blocks,1),1);   % works if only one stimtype
% For multiple stimtypes assign an equal number of stimtypes dependent upon
% how many stimtypes and how many blocks (exp)
count=1;
for i=1:size(all_exp_blocks,1)/length(params.blocks.stimtype)
    for j=1:length(params.blocks.stimtype)
        blocks.stimtype{count,1}=params.stimtype{j};
        count=count+1;
    end
end
...
for q = 1:length(params.blocks.stimtype)
    if params.blocks.npassive
        %         blocks.stimtype(end+1,1) = repmat({params.stimtype},params.blocks.npassive,1);  % Again, only works with one stimtype
        % for more than one stim type
        for i=1:length(params.blocks.npassive)
            blocks.stimtype{end+1,1}=params.stimtype{q};
        end
...
% must change the distnace between each stimulus for different stimulus
for i=1:trials.n
    switch trials.stimtype{i}
        case 'orientation'
            params.stim_min_dist(i) = params.stim.orientation.min_dist;
        case 'letters'
            params.stim_min_dist(i) = params.stim.letters.min_dist;
            % add in more cases if we want different stimulus
    end
end
...
for it = 1:trials.n
    ...
    left_td_pos_deg = PickPoints(params.stim_per_hemifield,[],params.stim_min_dist(it),[params.hemifield_width_deg params.hemifield_height_deg]);
    left_td_pos_deg(:,1) = left_td_pos_deg(:,1) - params.hemifield_eccen_deg; % offset xs
    right_td_pos_deg = PickPoints(params.stim_per_hemifield,[],params.stim_min_dist(it),[params.hemifield_width_deg params.hemifield_height_deg]);
...
AddText(w, 'Please maintain fixation throughout run', [xc yc], params.text_color, 7, params.instruction_text_size);
AddText(w, 'Before each block (6 trials per block) a cue will appear to notify you of the stimulus type and location', [xc yc], params.text_color, 5.8, params.instruction_text_size);
AddText(w, 'If they cue is black it is a letters trial, if it is white is an orientation block', [xc yc], params.text_color, 4.6, params.instruction_text_size);
AddText(w, 'Peripheral cue = memory task on one side or either', [xc yc], params.text_color, 3.3, params.instruction_text_size);
AddText(w, '(Colored circle will appear on the left, right, or both indicating where to attend)', [xc yc], params.text_color, 2.4, params.instruction_text_size);
AddText(w, 'If target changed press Right Shift, otherwise press Left Shift (last(red) or first(blue) if using num pad)', [xc yc], params.text_color, 1.2, params.instruction_text_size);
AddText(w, 'Central cue = passive task (no memory)', [xc yc], params.text_color, 0, params.instruction_text_size);
AddText(w, '(Ring around fixation cross will appear meaning a passive fixation trial)', [xc yc], params.text_color, -1.2, params.instruction_text_size);
AddText(w, 'You will have 3 seconds to respond after the second stimulus appeats', [xc yc], params.text_color, -2.4, params.instruction_text_size);
AddText(w, 'Press any key when ready to begin', [xc yc], params.text_color, -5, round(.80 * params.instruction_text_size)); % slightly smaller...
Screen(w,'Flip');
% Look for key press to signal a screen capture
while 1
...
for it = 1:trials.n;
    ...
    if trials.block(it,1) ~= this.block
        ...
        % added in stimtype specific switches
        if blocks.task(ib,1) == 1
            switch trials.stimtype{it}
                case 'orientation'
                    Screen('FrameOval',w,params.cue.color(1,:),cue.rect(3,:),6,1);
                case 'letters'
                    Screen('FrameOval',w,params.cue.color(2,:),cue.rect(3,:),6,1);
            end
        else
            switch trials.stimtype{it}
                case 'orientation'
                    switch blocks.mem_side(ib,1)
                        case -1 % targets on the left
                            Screen('FillOval',w,params.cue.color(1,:),cue.rect(1,:));
                        case 1 % targets on the right
                            Screen('FillOval',w,params.cue.color(1,:),cue.rect(2,:));
                ...
                case 'letters'
                    switch blocks.mem_side(ib,1)
                        case -1 % targets on the left
                            Screen('FillOval',w,params.cue.color(2,:),cue.rect(1,:));
                        case 1 % targets on the right
                            Screen('FillOval',w,params.cue.color(2,:),cue.rect(2,:));
                        case 2 % targets on either side
                            Screen('FillOval',w,params.cue.color(2,:),cue.rect(1,:));
                            Screen('FillOval',w,params.cue.color(2,:),cue.rect(2,:));
                        otherwise
...
% Changed to handle differnt accuracies for multiple people
perf.orientation.overall = 100 * mean(data.status(and(trials.task==2,strcmp(trials.stimtype,'orientation'))));
perf.letters.overall = 100 * mean(data.status(and(trials.task==2,strcmp(trials.stimtype,'letters'))));
% didn't add in a seperate function for stim type for per.correct_detect
perf.correct_detect = 100 * mean(data.status(trials.targ_change));
AddText(w, 'Run Complete!', [xc yc], params.text_color, 1, params.instruction_text_size);
AddText(w, sprintf('Performance For Orientation = %.02f%%',perf.orientation.overall), [xc yc], params.text_color, -1, params.instruction_text_size);
AddText(w, sprintf('Performance For Letters = %.02f%%',perf.letters.overall), [xc yc], params.text_color, -2, params.instruction_text_size);
...
for i=1:counter
    imwrite(imagetemp(i).image,sprintf('%s%s%s','/Users/C-Lab/Google Drive/Lab Projects/Marians Stuff/wmIPS/Methods_Results/Figure Stuff/Trial Sequence',imageName{i},'.jpeg'),'jpg')
end
