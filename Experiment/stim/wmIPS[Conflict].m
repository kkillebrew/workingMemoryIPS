...
params.stimtype = 'letters'; % type of stimulus (currently, for entire run)
...
switch params.stimtype
    ...
    case 'letters'
        ...
        params.stim.letters.letter_size = 60; % size of letters through TextSize
        params.stim.letters.letter_font = 'Arial Bold';

        params.stim_min_dist = 1; % min distance between two stimuli within the same hemifield
...
trials.targ_change_idx = trials.targ_change; % if target orient and positions are chosen randomly (see below), then this doesn't need to be random.  we can always count the first targ as the one that changes (if there is a chance).  so this can just replicate targ_change
for it = 1:trials.n
    ...
    if trials.targ_change(it,1)
        ...
        tmp_targ_idx = trials.targ_change_idx(it,1); % index of target that will change
        ...
        trials.targ_feature_probe{it,:}(tmp_targ_idx,:) = params.stim.(trials.stimtype{it,1}).feature_values(tmp_probe_feat_idx,:);
    end    
