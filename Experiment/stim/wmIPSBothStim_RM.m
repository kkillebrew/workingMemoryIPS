...
%         params.kb_dev_id = kb_id(strcmp(kb_names,'Apple Keyboard')); % use -1 for all devices, or specify a specific device
        ...
        % size
        params.stim.letters.letter_size = 80; % size of letters through TextSize
...
AddText(w, 'If target changed press Left Shift, otherwise press Right Shift (first(blue) or last(red) if using num pad)', [xc yc], params.text_color, 1.2, params.instruction_text_size);
    ...
    if trials.block(it,1) ~= this.block
        ...
        if blocks.task(ib,1) == 1
            switch trials.stimtype{it}
                case 'orientation'
                    Screen('FrameOval',w,params.cue.color(1,:),cue.rect(3,:),1,1);
                case 'letters'
                    Screen('FrameOval',w,params.cue.color(2,:),cue.rect(3,:),1,1);
            end
        else
            switch trials.stimtype{it}
                case 'orientation'
...
perf.orientation.overall = 100 * mean(data.status(strcmp(trials.stimtype,'orientation')));
perf.letters.overall = 100 * mean(data.status(strcmp(trials.stimtype,'letters')));
