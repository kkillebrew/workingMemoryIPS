...
%         params.kb_dev_id = kb_id(strcmp(kb_names,'Trainer (R1053)')); % use -1 for all devices, or specify a specific device
        ...
        % size
        params.stim.letters.letter_size = 55; % size of letters through TextSize
...
% with more stimtypes add more else statements
for i=1:trials.n
    if strcmp(trials.stimtype{i},'orientation')
        data.orientation.status(i) = data.status(i);
    elseif strcmp(trials.stimtype{i},'letters')
        data.letters.status(i) = data.status(i);
    end
end
% Changed to handle differnt accuracies for multiple people
perf.orientation.overall = 100 * nanmean(data.orientation.status);
perf.letters.overall = 100 * nanmean(data.letters.status);
...
AddText(w, sprintf('Performance For Orientation = %.02f%%',perf.letters.overall), [xc yc], params.text_color, -2, params.instruction_text_size);
