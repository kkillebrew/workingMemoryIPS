...
#      4. Copy final surface files to ../analysis/ (will be input to GLM)
echo "== $0 starting"
...
foreach run ($ALLRUNS)
	...
	foreach hs ($HEMIS)
	    # project to surface
	    echo; echo "projecting $hs data to surface for run $run ($sm mm sm)"
	    3dVol2Surf \
	    ...
	    mv -f ${SUBJ}_${run}${curfunc}.$hs.niml.dset ../analysis/
	end
    end
end
