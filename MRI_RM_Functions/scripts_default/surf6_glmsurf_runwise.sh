...
#
# This script will execute a surface-based GLM, separately for each run
#
...
foreach run ($ALLRUNS)
	...
	set output_prefix = ${SUBJ}_${run}${curfunc}_bucket.${hs}; # do NOT include .niml.dset
	set output = ${output_prefix}.niml.dset
	if ( -e $output ) then
