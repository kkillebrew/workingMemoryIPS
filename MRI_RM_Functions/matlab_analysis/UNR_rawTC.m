function [TC, subj] = UNR_rawTC(subj_initials,runs,rois,hemisphere,vox_select,varargin)
% rawMVPA
...
%                            'pre' to load pre-normalized data (must define pre-norm data using datafile_prefix)
%                            **NOT SETUP** 'grand' to normalize to mean activity in baseline_idx across entire run
%       datafile_prefix ['tscvrdtm'] = define prefix datafiles (no subj or runs), which defines preprocessing steps.
%       bucket_prefix   ['tscvrsm4_norm_bucket'] = prefix (no subject, but including smoothing and norm) for bucket used by voxel selection
%                         (nvox or tstat).
...
%
%       USAGE EXAMPLE: rawTC(... ,'datafile_prefix','mytimecourse','reg_include',{'cond1' 'cond2'})
%
% Return
...
%       04.14 - trying to adapt for UNR setup, which lacks subject-database
%               and the idea of "tasks".  simplified by making datafile and bucket
%               prefix explicitly defined.

% TODO


%% hardcoded params
% may need to update on a per-experiment basis
TRs = 2; % TR in seconds
...
if ismember(vox_select{1},{'fullFstat' 'tstat' 'tstatabs' 'nvox' 'nvoxabs' 'minbeta'})
    ...
    exp_bucket_file = ['_' options.bucket_prefix];
...
for i = runs
    fns{i} = fullfile(filedir,sprintf('%s_r%02d_%s+orig',subj_initials,i,options.datafile_prefix));
end
...
%% regressors
analysis_type = options.output_tail; % HACK: using output_tail to define stimtimes on a blockwise or trialwise case
[st] = UNR_ExtractStimTimes(subj_initials,analysis_type); % should provide a stimtimes structure, with a field for each regressor as a cell array of times for each run
...
for r = 1:length(rois)
    ...
    % keep track of the number of voxels in the anatomically-defined ROI
    % (i.e., without voxel selection)
    TC.nvox_anat.(this.roi_name) = sum(this.roi_mask(:));
    
    % check for anatomically-valid voxels
    if ~sum(this.roi_mask(:))
        TC.nvox.(this.roi_name) = 0;
        TC.nvox_prop.(this.roi_name) = 0;
    ...
    TC.nvox_prop.(this.roi_name) = TC.nvox.(this.roi_name) / TC.nvox_anat.(this.roi_name); % ...and proportion relative to anatomical ROI
...
%     TC.grand.avg_norm.(this.roi_name)  = repmat(NaN,1,TC.nTRs); % 1 X nTRs
    ...
    for i = 1:nRegs
        ...
        for b = unique(this_blocklabels)
            ...
            if b ~= 0
                ...
                TC.each.raw.(this.roi_name)(byreg_bi,:,i) = nanmean(this_pat(:,reg_idx)); % this is for a single event/block, sorted by regressor
        ...
        switch options.norm_type
            ...
            otherwise
                error('no other normalization type except ''each'' and ''pre'' is setup yet')
        end
    end
    
    % for grand data (**NOT SETUP YET**), if we have already normalized to 'each' event/block, we can just average over each.raw_norm
    ...
    % for 'each' and 'pre' we can just average the data we have...
    switch options.norm_type
        case {'each' 'pre'}
