...
% 11.29.11        - added 'notes' field to options/defaults as a placeholder for arbitraty notes to include in a datafile.

% TO DO - add a inter_cycle_blank, an amount of time to simply pause everything between cycles.  gives the ability to have blanks between eccen cycles.  alternatively, need to have a more explicit definition of ring.cut_in (instead of the current hack using wedge.cut_in).  ideally should not have targets occurring during these blank periods
...
try
    ...
    %% Default Paramters
    % default parameters can be changed through the options structure input.  For example:
    %    options.subj = 'test';
    %    options.stimulus = 'checker';
    %    options.direction = {'cw'};
    ...
    defaults.notes              =   ''; % place holder for any special notes that you want to provide for the datafile
    curclock = clock;
    ...
    defaults.key.dev_id             =   id(2);         % device id for checking responses.  -1 for all devices (but might be slow and miss some responses)
    defaults.key.quit_key           =   20;         % Q for quit
    defaults.key.resp_key           =   KbName('1!');         % response button, 44 for Keyboard, 33 for button box
    defaults.trigger_number   =   KbName('5%');          % cut out n deg (radius) from center. set to zero for no cut_out.
    ...
    for currun = 1:nRuns;
        ...
        Screen('Flip',w,[],1);
        % wait for  button response to indicate subject ready to roll
        while 1
            [keyIsDown, secs, keycode] = KbCheck(key.dev_id); %check response
            %             if keyIsDown
        ...
        while(GetSecs-start_time < ncycles * cycle_duration) && ~key.quit
            ...
            switch transition
                case 'smooth'
                    ...
                    if any(strcmp(options.direction(currun), {'ccw', 'in'}))
            ...
            switch direction{currun}
                ...
                case {'out' 'in'}
                    ...
                    % 11.29.11 - TESTING ADDITIONAL CUT INS FOR ECCEN
                    Screen('FrameArc', w, bg_color, corner_rect,0,360,corner_radius-(wedge.cut_in),corner_radius-(wedge.cut_in)); % mask everything beyond wedge.cut_in - defines max eccentricity shown
                 otherwise
        ...
        this_behav.accuracy    = 100 * this_behav.hits/this_behav.total_targs;
        ...
        Screen('FillRect',w, bg_color); % fill background
        txt1 = sprintf('Your accuracy is: %.02f', this_behav.accuracy);
        txt2 = sprintf('You had %d false alarms', this_behav.false_alarms);
