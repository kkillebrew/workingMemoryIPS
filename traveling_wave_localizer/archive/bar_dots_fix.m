...
options.stimulus = 'dots'; % type of visual stimulus.  'checkerboard' or 'dots'
...
options.dots.fcoh  = .75;	        % fraction of coherent dots moving in same direction.  these dots change direction for peripheral target when using dots stimuli.
...
% Attention Task ----------------------------------------------------
options.attend_task         =   1;             % 1 for fixation dimming task, 2 for peripheral (ring/wedge/dots)
options.dim_value           =   0*[137 137 137]; % dim color (0-255 [R G B]) for checkerboard attention task
options.dim_length          =   0.5;          % duration (sec) of fixation and ring/wedge dimming events
...
options.duty                = 0.0833;   % duty cycle (0.125 = 12.5%): fraction of cycle time that stimulus is visible at any one point.  For wedge, angular size is 360 * duty.
options.direction           = {'bar'};        % cell array of strings, one for each run.  options are wedge:'cw','ccw'     ring:'in','out'     bar:'bar'
...
options.bar.angles        =   angs;  % bars move perpendicular to the bar.angles vectors.  0 is top-to-bottom, 90 is right-to_left, 45 is upperRight-to-lowerLeft, etc. use NaN for fiation-only (no stimulus, null) cycles.  Must be the same length as ncycles
