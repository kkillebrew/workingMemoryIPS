...
options.direction   =   {'ccw';};  % Direction(s); # of directions = # of runs, choices: cw, ccw in, out
options.rot_period  =   40;                     % Length of individual cycle (in seconds): default 40
options.ncycles     =   1;                      % # of cycles per run: default 8
...
options.scaling = 1;                            % 1 for linear, 2 for logarithmic, 3 log-based cortical magnification ala Boynton & Duncan, 2002: defualt 3    
options.init_blank_time = 0;                   % start of run blank time: default 20?
options.end_blank_time  = 0;                    % end of run blank time: default 0?
...
options.dim_length  =   .2;                    % dim length (in seconds): can change to make task easier
options.dim_value   =   [0 0 0]%127 127 127];          % dim color value (RGB)

options.min_dim     =   .2;          % minimum time between fixation point dims
options.max_dim     =   .2;          % max time between fixation point dims
