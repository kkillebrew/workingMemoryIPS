...
try
    ...
    defaults.stimulus  = 'checker'; % type of visual stimulus.  'checkerboard', 'dots', or 'solid'

    % options for checkerboard stimulus
    defaults.checker.image_size         = 'max';        % size (deg radius) of background checkerboard image or 'max' to fill screen with image.  Leave at 'max' if unsure, better to scale mask directly.
    ...
    defaults.dots.color = [255 255 255]; % [R G B] color of each dot.  Can define color separately for each dot in an 3xNDOTS                matrix
    ...
    defaults.dim_value            =   [137 137 137]; % dim color (0-255 [R G B]) for checkerboard attention task
...
% %     defaults.attend_dots_speed    =   1;   % amplitude of speed (pix/sec) change (randomly +/-) in all dot motion for peripheral target
    ...
    defaults.transition           = 'smooth';  % 'smooth' for continuous updating of position and smoothly moving mask, 'discrete_steps' for static mask positions cycling in normal order, 'discrete_random' for each step being randomly selected from all steps/cycle pairs
    defaults.discrete_step_size   = 'calculate'; % (sec) if discrete_step_size is true, how long does the mask stay in one position?  in other words, at what time is the mask updated to step to the next position?  if 'calculate' the discrete_step_size will be calculated such that each point on the screen is part of one and only one step
    defaults.discrete_step_offset = 'half_step_size'; % time offset (sec) to shift each discrete step by.  If 0, the first step will not show a simulus (or rather a sliver of one).  By default, it is 'half_step_size', which will be calculated as discrete_step_size/2.
    ...
    defaults.bar.angles        =   (0:(360/defaults.ncycles):(360-360/defaults.ncycles));  % bar.angles defines the starting position of the bars center. 0 up, positive clockwise.  use NaN for fiation-only (no stimulus, null) cycles.  bars move perpendicular to the bar.angles vectors.  size(bar.angles,2) must equal ncycles. size(bar.angles,1) should be 1 (to use the same angle progression for all runs), or length(direction) to use a unique angle progression for each run.
    ...
    defaults.bar.cut_in        =   NaN;        % cut in n deg (radius) from outer max; equals max eccen of wedge.  set to NaN for no cut_in.
    ...
    if strcmp(options.discrete_step_size,'calculate')
        options.discrete_step_size = options.cycle_duration * options.duty;
    end
    if strcmp(options.discrete_step_offset,'half_step_size')
        options.discrete_step_offset = options.discrete_step_size/2;
    end    
    ...
    if strcmp(transition,'discrete_random')
        ...
        steps_per_cycle = options.cycle_duration/discrete_step_size;
    ...
    switch stimulus
        case 'checker' % flashing/alternating checkerboard images
            ...
            % Resize images
            if strcmp(checker.image_size,'max')
    ...
    for currun = 1:nRuns;
        ...
        fix.this_color          = fix.color; % start with undimmed color
        targ_on                 = 0;
        ...
        % wait for trigger (or button response) to start experiment
        while ~keyIsDown
            [keyIsDown, secs, keycode] = KbCheck(key.dev_id); %check response
        ...
        while(GetSecs-start_time < ncycles * cycle_duration) && ~key.quit
            ...
            switch transition
                ...
                case 'discrete_steps'
                    ...
                    if cur_step ~= last_step
                        ...
                        time_frac = mod(cur_time, cycle_duration)/cycle_duration; % fraction of cycle we've gone through
                        time_frac = time_frac + discrete_step_offset/cycle_duration; % add offset for discrete mask changes
                        if any(strcmp(options.direction(currun), {'ccw', 'in', 'bar'}))
                            time_frac = 1-time_frac; % reversing time for these conditions
                        end
                        last_step = cur_step;
            ...
            if ~targ_on && GetSecs > timing.next_fix  % if not currently "dimmed" and time for next fixation dimming...
                % dim fixation cross
                switch attend_task
                    case 1 % FIXATION TASK
                        % ...update fixation spot color
                        fix.this_color = fix.dim_color;
                ...
                switch attend_task
                    ...
                    case 2 % PERIPHERAL TASK
                        ...
                        switch stimulus
                            ...
                            case 'dots'
                                % if we are using dot speed as target, need to reset dot speed
                                dots.speed_pps = dots_speed_orig * ppd; % normal dot speed (pix/sec)
                        end
                end
        ...
        behav.runtime(currun)  = GetSecs-start_time; % Calculate total runtime
        this_behav.accuracy    = this_behav.hits/this_behav.total_targs;
        behav.accuracy(currun) = this_behav.accuracy;
        behav.hits             = this_behav.hits;
        behav.false_alarms     = this_behav.false_alarms;
        behav.total_targs      = this_behav.total_targs;
...
Screen('FillRect', w, fix.color, fix.rect); % outer box, or solid square if size_inner is zero
if fix.size_inner > 0
