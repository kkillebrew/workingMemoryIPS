...
try
    ...
    defaults.behav              =   [];
    defaults.savefile           =   1; % boolean, save output file?
    
    % Screen, Display, Response
    ...
    defaults.bg_color           =   0;          % background color on 0-255 grayscale
    ...
    % fixation point
    defaults.fix_color          =   [255 0 0]; % color of fixation spot
    defaults.fixdim_color       =   [255 255 255]; % color of fixation spot for fixation-dimming events (see defaults.atten_task)
    ...
    defaults.teststruc.dots.ndots  = 1;
    defaults.teststruc.dots.fcoh  = 0.19;
    defaults.teststruc.ring  = [9 8 7];
    defaults.teststruc.wedge = 'ryan';
    ...
    transition = 'smooth'; % 'smooth' for continuous updating of position and smoothly moving mask, 'discrete_steps' for static mask positions cycling in normal order, 'discrete_random' for each step being randomly selected from all steps/cycle pairs
    % discrete_steps      = 0; % boolean, should we use discrete steps?  if false, continuous updating on each frame for smooth stimulus
    ...
    %% Merge Defaults and Options, setup local variables
    ...
    % --- Local Variables that can be determined now that options have been fully parsed ---
    ...
    if strcmp(transition,'discrete_random')
        % create a random list of all cycle-step pairs that we can randomly step through
        steps_per_cycle = options.rot_period/discrete_step_size;
        [tmp_c tmp_s] = meshgrid(1:ncycles,1:steps_per_cycle);
        cycle_steps = [tmp_c(:) tmp_s(:)]; % all possible cycle-step pairs
        rand_idx = shuffle(1:size(cycle_steps,1));
        cycle_steps = cycle_steps(rand_idx,:);
    end
    ...
    bar_mask       = repmat(bg_color,256,256); % an image of a rectangle
    ...
    bar_angles_deg = [0:45:359]; % bars move perpendicular to the bar_angle vector.  0 is top-to-bottom, 90 is right-to_left, 45 is upperRight-to-lowerLeft, etc. use NaN for fiation-only (no stimulus, null) cycles *** SHOULD BE MOVED TO OPTIONS/DEFAULTS - needs to be linked to NCYCLES***
    

    if stim == 3
        p.norm_speed 	= 7;	% normal dot speed (deg/sec)
        p.ndots 		= 15000;	% number of dots
        ...
        %p.switch_coh_min = 1;
        %p.switch_coh_max = 1;
        p.fcoh = 0.50;	% fraction of coherent dots
...
% % %         rmin = 0; 	% minimum
    ...
    for current_run = 1: length(options.direction);
        ...
        this_fix_color          = fix_color; % start with undimmed color
        fix_dimmed              = 0;
        behav.falarms           = 0;
        behav.hits              = 0;
        behav.total_dim         = 0;
        ...
        Screen('FillRect',w, fix_color, [xc-fp_size; yc-fp_size; xc+fp_size; yc+fp_size]);   % set fixation point
...
% % %                 xy = [r r] .* [cos(t), sin(t)];	% dot positions in Cartesian pixel coordinates relative to center
        ...
        while(GetSecs-start_time < ncycles * rot_period) & ~key.quit
            ...
            switch transition
                ...
                case 'discrete_steps'
                    ...
                    cur_step = floor(cur_cyctime/discrete_step_size)+1; % relative to the current cycle
                ...
                case 'discrete_random'
                    ...
                    % pull cur_cycle and cur_step from cycle_steps
                    cur_cycle = cycle_steps(cur_cycstep_idx,1);
                    cur_step  = cycle_steps(cur_cycstep_idx,2);
                    
                    if cur_step ~= last_step
                        % then this is a new step so update time_frac
                        % N.B. we need to update time_frac DIRECTLY to force it to the proper step within this cycle
                        %      to account for the random nature of discrete_random
                    ...
                    % we forcefully update time_frac in this switch if necessary, so never do it outside this switch
                    update_time_frac = 0;
            ...
            if fix_dimmed & GetSecs > fix_start+dim_length % if fix dimmed and length of fix dim has passed (.35) then undim
                fix_dimmed = 0;
                if atten_task == 1
                    this_fix_color = fix_color; % white
                end
            ...
            switch stim
                ...
                case 3 % random dot motionq
                    ...
                    Lk = find(rand(p.ndots,1) < p.fkill); % locus of all dots that have gone beyond borders or are killed (including a random selection that will always die each frame)
                    nLk = length(Lk);
                    if nLk
                        % choose new dot positions for killed dots
                        ...
                        dxdy(Lk,:) = pfs * [cos(mdir(Lk)), sin(mdir(Lk))];
                    end
            ...
            switch direction{current_run}
                case {'cw' 'ccw'}
                    ...
                    Screen('FillArc', w, bg_color, [xc-stim_r, yc-stim_r, xc+stim_r, yc+stim_r], stim_angle+(0.5*wedge_size), 360-wedge_size); % pac-man mask to define wedge
                    Screen('FrameArc', w, bg_color, [xc-max_viewing, yc-max_viewing, xc+max_viewing, yc+max_viewing],0,360,max_viewing-(cut_in),max_viewing-(cut_in)); % mask everything beyond cut_in - defines max eccentricity shown
                                        
                    Screen('FillArc', w, bg_color, [xc-(cut_out), yc-(cut_out), xc+(cut_out), yc+(cut_out)], 0, 360); % central circular mask defining minimum eccentricity shown
                    Screen('FillRect',w, this_fix_color, [xc-fp_size; yc-fp_size; xc+fp_size; yc+fp_size]); % fixation spot
                ...
                case {'out' 'in'}
                    ...
                    Screen('FrameArc', w, bg_color, [xc-max_viewing, yc-max_viewing, xc+max_viewing, yc+max_viewing],0,360,max_viewing-r_outer_pix,max_viewing-r_outer_pix); % mask everything beyond r_outer_pix - defines rings outer edge
        ...
        Screen('FillRect',w, bg_color); % fill background
        Screen('FillRect',w, fix_color, [xc-fp_size; yc-fp_size; xc+fp_size; yc+fp_size]); % fixation spot
        Screen('Flip', w);
