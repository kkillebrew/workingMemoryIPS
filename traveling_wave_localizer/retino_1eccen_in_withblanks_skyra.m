...
options.key.dev_id             =   id(1); % device id for checking responses.  -1 for all devices (but might be slow and miss some responses)
options.trigger_number         =   KbName('5%'); % the actual trigger number

% Datafile --------------------------------
options.subj               =   'msco';
options.notes = 'using hacked version of TravelingWave that allows using wedge.cut_in during ring mask.  this gives us the ability to produce 40 s cycles with 10 s gaps between cycles';
...
options.cycle_duration     =   50;         % duration (sec) of a single cycle (eg, rotation period of wedge or ring). *** should be an increment of the TR ***
options.init_blank_time    =   10;          % initial blank time (sec)
options.end_blank_time     =   10;          % blank time at end (sec)
...
options.attend_checker_wedge =   30;         % degress of atten wedge size for ring mask
...
options.direction           = {'in'}; % cell array of strings, one for each run.  options are wedge:'cw','ccw'     ring:'in','out'     bar:'bar'
...
options.ring.outer_deg_max =   16.875;      % outer maximum stimulation point for ring
