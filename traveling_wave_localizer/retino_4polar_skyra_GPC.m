...
options.key.dev_id             =   id(2); % device id for checking responses.  -1 for all devices (but might be slow and miss some responses)
...
options.dim_value           =   0*[75 75 75]; % dim color (0-255 [R G B]) for checkerboard attention task
...
options.direction           = {'ccw' 'cw' 'ccw' 'cw'}; % cell array of strings, one for each run.  options are wedge:'cw','ccw'     ring:'in','out'     bar:'bar'
